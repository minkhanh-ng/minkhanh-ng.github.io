---
title: 'TcHMI show case of a Vibration monitoring system'
date: 2024-04-20
permalink: /posts/2024/04/20/TcHMI show case of a Vibration monitoring system
tags:
  - TwinCAT 3
  - TE2000
  - HMI
  - Visualization
---

![System Overview](https://enginine.wordpress.com/wp-content/uploads/2025/09/media1-ezgif.com-video-to-gif-converter-1.gif?w=800)

> A turbine pump using analogue sensors for winding and bearing temperature, keyphasor for rotating speed, and velocity sensors for vibration. The program is an implementation of TE2000 and TwinCAT 3, with a custom Fourier function (implemented in case the sensors are not RMS).

### Main Pages Overview
Go around the main pages of the HMI:

[![Main Pages Walkthrough](https://img.youtube.com/vi/Bw2JSHlSSPU/0.jpg)](https://youtu.be/Bw2JSHlSSPU)

### Component Details
Component detail values and data can be monitored and modified quickly via pop-ups.

[![Component Details](https://img.youtube.com/vi/JnEGradS4OU/0.jpg)](https://youtu.be/JnEGradS4OU)

### Other Features
Additional functionalities included in the project:

[![Other Features](https://img.youtube.com/vi/kOtfw7BJ3sg/0.jpg)](https://youtu.be/kOtfw7BJ3sg)

### Hardware Implementation
Some features only work on real hardware. *Note: Video of the working HMI on-site is prohibited.*

![HMI Animation](https://enginine.wordpress.com/wp-content/uploads/2025/09/media2-ezgif.com-video-to-gif-converter.gif?w=800)

---

**Source Code:** [https://github.com/KowalskiPi/Tc3_VN001007_VMS](https://github.com/KowalskiPi/Tc3_VN001007_VMS)
